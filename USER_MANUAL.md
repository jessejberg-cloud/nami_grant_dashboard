...
Revision date: 2026-09-11. Application documentation release: onboarding-v1, based on source 8ea3311. The commit containing this manual identifies the complete release. This guide covers the grant dashboard, not the separate financial dashboard.
...
5. [Requirements and tasks](#requirements-and-tasks)
6. [Owners deadlines and dependencies](#owners-deadlines-and-dependencies)
7. [Evidence and confidence](#evidence-and-confidence)
8. [Financial snapshots](#financial-snapshots)
9. [Issues and escalation](#issues-and-escalation)
10. [Grant Radar](#grant-radar)
11. [Activity history](#activity-history)
12. [Import records](#import-records)
13. [Export and reset](#export-and-reset)
14. [Preferences and integrations](#preferences-and-integrations)
15. [Troubleshooting](#troubleshooting)
16. [Worked sample examples](#worked-sample-examples)
17. [First week checklist](#first-week-checklist)
18. [Administrator adoption checklist](#administrator-adoption-checklist)
...
Choose Explore the sample workspace to continue with fictional records. Review agency setup opens Settings for information only; it does not connect accounts or approve real data. The orientation explains records, trust indicators and available settings, then offers Open sample government grant for a read-only practice action. Open Government monthly expenditure report within that grant, inspect Owner and Source or evidence, and open its Depends on record in the Next step panel. If the original sample is absent, inspect another sample or deliberately reset samples after exporting changes.
...
Choose Overview to return to Needs Attention. Open any row in Attention queue to inspect details and the written reasons for its priority. The queue considers the selected workspace only. It includes issues, deadlines, missing owners, missing evidence, dependencies and uncertain information.
...
1. Choose Grants in the navigation.
2. Use Search titles, owners, notes to narrow the list. Search applies to the current list and workspace; it does not search evidence documents or all fields.
3. Select a grant row to open Record details and its Requirements, tasks & issues panel.
4. Choose Edit record, change the relevant fields and Save record. A rejected save keeps the form open with an explanation.

To create a grant, choose Add grant. Enter a Title, Owner, Due date and Status; grants default to Active, so explicitly choose Pending for unconfirmed funding. Enter Award amount and Recorded expenses only as synthetic/manual totals. Add source, notes, verification date and snapshot date when appropriate. Save record persists to the shared database.

To archive a grant, open it → Edit record → Status → Archived → Save record. Archiving retains the record; it is not deletion and does not automatically archive its requirements or tasks. Archived records still appear in lists and some totals. Use search to locate them; there is no separate archive browser or bulk archive action.

The Back to button returns to the current section, not necessarily the previously opened parent record. Navigation buttons clear the selected detail and search. Selecting a new workspace clears the detail but can retain an existing search, so clear search if expected rows are missing.
...
Open a grant, then Add requirement or Add task in its linked-record panel. This preselects the linked grant. Alternatively, use Compliance → Add requirement or Tasks → Add task and choose Linked grant yourself. Deadlines displays both types, while Compliance displays requirements only.
...
Open a record → Edit record → Source / evidence URL. Use an HTTP or HTTPS link. Save, then use Open source / evidence in Record details to open it in another tab. There is one source/evidence field per record; additional context can go in notes. The dashboard does not upload documents, check link permissions, read file contents or independently establish evidence sufficiency.
...
Open a grant → Edit record to change Award amount, Recorded expenses and Financial snapshot as of. No date means UNKNOWN; more than 30 days old means STALE. Editing unrelated fields does not refresh financial freshness. Dates cannot be in the future. Amounts must be nonnegative numbers no greater than one billion dollars.
...
For batches larger than 100, split carefully. Import-generated IDs differ from input IDs; later batches must use the resulting destination IDs for earlier records. The download template is a single opportunity example, not a full template for every record kind. INTEGRATION.md contains the complete field contract and statuses.

## Export and reset

Settings → Export records downloads all records in the selected workspace in a version 2 JSON envelope. Save a copy before significant sample changes. The file excludes activity, onboarding state and browser selections. It is a record export, not a complete database backup.
...
**Rejected save:** Read the error inside the open form. Check required title, allowed status, nonnegative amounts, real calendar dates, HTTP(S) source links, completed prerequisites and escalation owner. Correct the field and Save record. Your form remains available on a rejected save.
...
**Import failure:** Confirm a JSON object containing records, numeric schemaVersion 1 or 2, 1–100 records and all required numeric fields. Check duplicate titles/source combinations, IDs, links and dates. A financial Export summary array is not a record-import envelope. Correct and select the file again.

**Missing record or information:** Check the workspace, current section and search text. Archived records remain in lists but disappear from attention flags. Inspect the owner, deadline, source and verification dates; missing values are not proof the underlying work is unnecessary.
...
- Day 2: Inspect each sample grant, its owner, deadlines and linked commitments. Note missing information.
- Day 3: Practice the blocked-report example. Explain why task completion and requirement completion differ.
- Day 4: Review evidence confidence, financial freshness and the escalated issue separately. Identify who would verify each source in an agency deployment.
...
- Approve the data inventory, retention, confidentiality rules and verified award terms. Exclude participant/client-level records from this prototype scope.
